...
const DataSourceTemplate = () => {
  ...
  const [modalTitle, setModalTitle] = useState('新增模版')
      ...
      render: (text: string, record: any) => (
        <div>
          <a
            ...
            onClick={() => viewTemplate(record)}>
            查看
          </a>
          <Divider type="vertical" />
          <a style={{ color: '#1890ff' }} onClick={() => editTemplate(record)}>
            编辑
          </a>
          <Divider type="vertical" />
   ...
   *  根据词查询
   */
  const searchTemplate = async (param: any) => {
    setLoading(true)
    const { res } = await requestFn(dispatch, state, {
  ...
  const search = (searchParams: IParams) => {
    searchTemplate({ word: searchParams.name })
   ...
   * 新增/编辑用户请求
   */
  const saveTemplate = async (param: any) => {
    console.log(param)
    setLoading(true)
    const { res } = await requestFn(dispatch, state, {
   ...
   * 删除用户请求
   */
  const deleteTemplate = async (word: string) => {
    setLoading(true)
    const { res } = await requestFn(dispatch, state, {
